...
function ManagePets() {
  ...
  useEffect(() => {
    if (statusFilter === "all") {
      ...
      // Show both lost pets and pets found by community (they stay in lost section)
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
        ...
        <p className="text-xs text-yellow-700">User ID: {user?.uid}</p>
        <p className="text-xs text-yellow-700">User Email: {user?.email}</p>
        <p className="text-xs text-yellow-700">Total Pets: {pets.length}</p>
        <p className="text-xs text-yellow-700">Registered Pets: {getStatusCount("registered")}</p>
        <p className="text-xs text-yellow-700">Current Filter: {statusFilter}</p>
        <p className="text-xs text-yellow-700">Filtered Pets: {filteredPets.length}</p>
        <p className="text-xs text-yellow-700">Check browser console for detailed pet data</p>
      </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={async () => {
              try {
                const allPetsQuery = await getDocs(collection(db, "pets"));
                const allPets = allPetsQuery.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                console.log("ALL PETS IN DATABASE:", allPets);
                alert(`Found ${allPets.length} total pets in database. Check console for details.`);
              } catch (error) {
                alert("Error fetching all pets: " + error.message);
              }
            }}
